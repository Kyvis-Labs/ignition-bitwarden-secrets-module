...
public class GatewayHook extends AbstractGatewayModuleHook {
    public static final String MODULE_ID = "com.kyvislanbs.secrets.bitwarden";

    private NamedResourceHandler<BitwardenSecretProviderResource> namedResourceHandler;

    @Override
    public void setup(GatewayContext context) {

        // Register our localized properties with BundleUtil
        BundleUtil.get().addBundle("BitwardenSecretProvider", getClass(), "BitwardenSecretProvider");

        // Register our named resource handler for the MongoDbSecretProviderResource type.
        namedResourceHandler = NamedResourceHandler.newBuilder(BitwardenSecretProviderResource.META)
                .context(context)
                .build();
    }
    ...
    public void shutdown() {
        BundleUtil.get().removeBundle("BitwardenecretProvider");
    ...
    public List<? extends ExtensionPoint<?>> getExtensionPoints() {
        return List.of(
                new BitwardenSecretProviderExtensionPoint()
        );
    }
}
